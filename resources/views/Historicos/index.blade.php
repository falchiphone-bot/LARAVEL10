    ...
    <div class="py-5 bg-light">
        <div class="container">
            ...
            <div class="card">
                ...
                <form method="POST" action="{{route('Historicos.index',$Historicos->ID)}}" accept-charset="UTF-8">
                    <input type="hidden" name="_method" value="PUT">

                <div class="col-4">
                    <label for="EmpresaID" style="color: green;">Empresas disponíveis</label>
                    <select required class="form-control select2" id="EmpresaID" name="EmpresaID"> --}}
                        <option value="">
                            Selecionar empresa
                        </option>
                        @foreach ($empresas as $EmpresasSelecionar)
                            <option @if ($historicos ?? null) @if ($historicos->ID == $EmpresasSelecionar->EmpresaID) selected @endif
                                @endif
                                value="{{ $EmpresasSelecionar->EmpresaID }}">
                        ...
                        <tbody>
                            @foreach ($Historicos as $Historico)
                                <tr>
                                    ...
                                    <td class="">
                                        {{ $Historico->ContaDebito->Descricao??null }}
                                    </td>
                                    <td class="">
                                        {{ $Historico->ContaCredito->Descricao??null }}
                                    </td>
