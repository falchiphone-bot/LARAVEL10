...
<head>
    <meta charset="utf-8">
    <title>Representantes (PDF)</title>
    <style>
        ...
        <table style="width: 100%; border: none; border-collapse: collapse;">
            <tr>
                ...
                <td style="border: none; text-align: right;">
                    <h1>{{ $headerTitle ?? 'Representantes' }}</h1>
            ...
            <tbody>
                @forelse($registros as $r)
                    <tr>
                        ...
                        <td>{{ $r->cpf }}</td>
                        <td>{{ $r->cnpj }}</td>
                        <td>{{ $r->agente_fifa ? 'SIM' : 'NÃO' }}</td>
                        <td>{{ $r->oficial_cbf ? 'SIM' : 'NÃO' }}</td>
                        <td>{{ $r->sem_registro ? 'SIM' : 'NÃO' }}</td>
                        <td>{{ optional($r->MostraEmpresa)->Descricao }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="9" class="small">Nenhum registro encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
