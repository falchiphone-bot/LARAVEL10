            ...
            <div class="card-body">
                ...
                @can('FUNCAOPROFISSIONAL - INCLUIR')
                    <a href="{{ route('FuncaoProfissional.create') }}" class="btn btn-primary btn-lg enabled" tabindex="-1" role="button"
                        aria-disabled="true">Incluir função profissional</a>
                @endcan
                <div class="card-header">
                    <div class="badge bg-info text-wrap" style="width: 100%;font-size: 24px">
                        <p>Total de função profissional cadastrados no sistema de gerenciamento administrativo e contábil:
                            {{ $model->count() ?? 0 }}</p>
                    </div>
                </div>
                        ...
                        @foreach ($model as $Model)
                            <tr>
                                ...
                                @can('FUNCAOPROFISSIONAL - EDITAR')
                                    <td>
                                        <a href="{{ route('FuncaoProfissional.edit', $Model->id) }}" class="btn btn-success" tabindex="-1"
                                ...
                                @can('FUNCAOPROFISSIONAL - VER')
                                    <td>
                                        <a href="{{ route('FuncaoProfissional.show', $Model->id) }}" class="btn btn-info" tabindex="-1"
                                ...
                                @can('FUNCAOPROFISSIONAL - EXCLUIR')
                                    <td>
                                        <form method="POST" action="{{ route('FuncaoProfissional.destroy', [$Model->id]) }}">
                                            @csrf
                                            @method('DELETE')
