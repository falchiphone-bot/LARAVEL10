...
class ExtratoCaixaController extends Controller
{
    public function __construct()
    {
        $this->middleware('auth');
        $this->middleware(['permission:EXTRATO CAIXA - LISTAR'])->only('ExtratoCaixa');
    }

    public function ExtratoCaixa(Request $request)
    {


        $DESCONSIDERAR_BLOQUEIOS = $request->DESCONSIDERAR_BLOQUEIOS;
        $DesmarcarConferido = $request->DesmarcarConferido;
        $criarlancamentos = $request->criarlancamentos;
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $Mensagem = null;
        ...
        $novadata = array_slice($cellData, 2);
        // $novadata = array_slice($cellData, 152);

        ///// CONFERE SE EMPRESA BLOQUEADA
        $Empresa = '1025';  //// TANABI ESPORTE CLUBE 2023
        ...
        foreach ($novadata as $PegaLinha => $item) {
            ...
            if ($criarlancamentos) {
                ...
                session([
                    'Lancamento' => 'Conferi lançamentos ou lancei!',
                ]);
            }
            else
            {
                session([
                    'Lancamento' => 'Não efetuado nenhum lançamento ou conferido!',
                ]);
            }
        }
